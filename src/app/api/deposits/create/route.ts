...
export async function POST(req: NextRequest) {
  try {
    ...
    if (lastDeposit) {
      ...
      const now = Date.now();
      if (now - last < 60 * 1000) {
        console.error("[DEPOSIT] Deposit too soon (1/minute rule)", { lastDeposit, now });
        return NextResponse.json({ error: "You can only deposit $1 per minute (testing mode)." }, { status: 403 });
      }
    }
