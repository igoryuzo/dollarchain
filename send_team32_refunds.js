...
async function processAndSendRefunds() {
  try {
    ...
    console.log(`\nTotal refund amount: ${totalRefundAmount.toFixed(2)} USDC for ${refunds.length} users`);
    
    if (DRY_RUN) {
      console.log('\n*** DRY RUN MODE - No transactions sent ***');
      console.log(`Would send ${refunds.length} refunds totaling ${totalRefundAmount.toFixed(2)} USDC`);
      return;
    }
